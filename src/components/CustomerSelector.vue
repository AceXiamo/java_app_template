...
const originalData = ref<Customer[]>([])
const inputValue = ref<string>('')
const selectValue = ref<Customer>({})
const showItems = ref<Customer[]>([])
const className = ref<'show' | 'hide' | ''>('')

function handleCheck(data: Customer) {
  if (props.ignore?.find(item => item.id === data.id))
    return
...
watch(inputValue, (value) => {
  showItems.value = originalData.value.filter(item => item.customerName!.includes(value) || item.customerPhone!.includes(value))
})
...
<template>
  ...
  >
    <view
      class="min-h-[650rpx] flex flex-col rounded-t-md bg-white p-[30rpx]" :class="[`${className}`]"
      @click.stop
    >
      ...
      <view class="mt-[20rpx] w-max flex-none rounded-1 bg-gray-100 p-[5rpx_10rpx]">
        <input
          v-model="inputValue"
          type="text"
          class="w-[380rpx] text-[26rpx] text-gray-500"
          placeholder="输入客户姓名或手机号以筛选"
        >
      </view>

      <view
        class="mt-[20rpx] h-0 flex flex-auto flex-col gap-[6rpx] overflow-auto"
        :style="{
      ...
      >
        <view class="h-0 flex flex-auto flex-col snap-y gap-[6rpx] overflow-y-auto">
          <view
            ...
            :class="[
              $props.ignore?.find((item: Customer) => item.id === data.id) && 'opacity-50',
            ]"
            @click="handleCheck(data)"
          >
            <view class="w-full flex items-center gap-[10rpx]">
              <view
                i-heroicons:check-circle-20-solid text-[22rpx]
                :style="{
                  color: selectValue?.id === data.id ? '#22c55e' : '#e5e7eb',
                }"
              />
        ...
        <view class="mt-auto flex items-center gap-[50rpx]">
          ...
          <view
            class="ml-auto w-max flex flex-none items-center rounded-1 p-[10rpx_30rpx] active:opacity-60" :class="[
              selectValue?.customerName && `bg-blue-500`,
              !selectValue?.customerName && `bg-blue-300`,
            ]"
            @click="confirm"
          >
